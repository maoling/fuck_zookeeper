...
public class PurgeTxnLog {
    ...
    /**
     * 参数好骚气！
     * @param txnLog 总共的事务日志,快照
     * @param snaps  需要保留的最近的N个事务日志,快照
     */
    // VisibleForTesting
    static void retainNRecentSnapshots(FileTxnSnapLog txnLog, List<File> snaps) {
        ...
        //leastZxidToBeRetain 最近的快照的ZXID
        ...
        class MyFileFilter implements FileFilter{
            ...
            //accept(接受),false才表示过滤
            public boolean accept(File f){
                if(!f.getName().startsWith(prefix + "."))
                    return false;
                long fZxid = Util.getZxidFromName(f.getName(), prefix);
                //如果该文件的zxid比最近的快照的ZXID还要大则过滤
        ...
        /**
         * files是需要删除的日志及快照,通过自定义MyFileFilter来过滤
         * 看看别人的代码写的多高级，高大上!
         */
